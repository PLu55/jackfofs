...
void test_process_queue(void)
{
  ...
  TEST_ASSERT_TRUE(fabs(q->slot[0]->fof[0].time - fof.time) < 1e-9);
  TEST_ASSERT_TRUE(fabsf(q->slot[0]->fof[0].argv[FOF_ARG_freq] - 1.0f) < 1e-9f);

  fof.time = slot_size / sample_rate * 2.5;
...
void default_fof(fof* _fof)
{
  ...
  _fof->argv[FOF_ARG_beta]   =     0.3f;
  _fof->argv[FOF_ARG_alpha]  =     2.5f;
